...
export const replaceChildren = (node : HTMLElement, html : HTMLElement) => {
    removeChildren(node).appendChild(html)
    return node
}
/**
...
export const parseStringCodepoints = (codepoints : string) : string => {
    if(/-/g.test(codepoints)){
        const arr = codepoints.split("-").map(str => parseInt(`0x${str}`))
        return String.fromCodePoint(...arr)
    }
    return String.fromCodePoint(parseInt(`0x${codepoints}`))
}

export const getCodepointsFromString = (string : string) : Array<number> => {
